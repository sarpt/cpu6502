...
mod ora {
    ...
    mod ora_im {
        ...
        fn should_or_accumulator_with_a_value_from_immediate_address() {
            let mut cpu = CPU::new(Box::new(MemoryMock::new(&[0x22, 0x00])));
            cpu.program_counter = 0x00;
...
mod brk {
    ...
    fn should_put_program_counter_incremented_by_one_and_processor_status_on_stack() {
        ...
        assert_eq!(cpu.memory[0x01FF], 0xCE);
        assert_eq!(cpu.memory[0x01FE], 0xAB);
        assert_eq!(cpu.memory[0x01FD], 0b11111111);
    }
